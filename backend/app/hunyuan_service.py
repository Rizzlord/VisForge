...
class HunyuanService:
    ...
    async def generate(self, image_data_url: str, params: HunyuanParams) -> tuple[bytes, dict[str, object]]:
        ...
        cmd = [python_executable, str(Path(__file__).resolve().parent / "hunyuan_worker.py")]
        ...
        stderr_task = asyncio.create_task(
            SubprocessLogger.log_stream(process.stderr, "Hunyuan3D", "generation")
        )
        ...
        if process.returncode != 0:
            SubprocessLogger.log_error("Hunyuan3D", "generation", f"Worker failed with return code {process.returncode}")
            raise RuntimeError(f"Hunyuan3D worker failed with return code {process.returncode}")
